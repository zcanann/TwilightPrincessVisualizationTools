namespace Twilight.Source.ProcessSelector
{
    ...
    using Twilight.Engine.Scanning;
    using Twilight.Engine.Scanning.Scanners;
    ...
    public class ProcessSelectorViewModel : ToolViewModel
    {
        ...
        public IEnumerable<Process> WindowedProcessList
        {
            get
            {
                ...
                return this.ProcessList?.Where(process => process.HasWindow() && (process?.Id ?? 0) != (this.SelectedProcess?.Id ?? 0) && (process?.ProcessName?.Contains("Dolphin") ?? false))
